...
export class DefaultHeaderComponent extends HeaderComponent {
  ...
  getUserCodeFromCookie() {
    const csrf = this.cookieService.get('data_csrf_cookies');
    if (csrf) {
      const stringCSRF = atob(csrf);
      this.jsonCSRF = JSON.parse(stringCSRF);
    } else {
  ...
  logout(){
    this.cookieService.delete('data_csrf_cookies');
